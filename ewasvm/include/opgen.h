...
namespace ewasvm
{
   ...
   int pwart_CheckAndGenStubFunction(ModuleCompiler *m, WasmFunctionEntry fn);
   ...
   int pwart_EmitStoreStackValue(ModuleCompiler *m, StackValue *sv, int memreg,
                                 int offset);
   // 32bit arch only
   int pwart_EmitSaveStack(ModuleCompiler *m, StackValue *sv);
   int pwart_EmitSaveStackAll(ModuleCompiler *m);
   int pwart_EmitLoadReg(ModuleCompiler *m, StackValue *sv, int reg);
   ...
   int pwart_EmitCallFunc(ModuleCompiler *m, Type *type, sljit_s32 memreg,
                          sljit_sw offset);
   int pwart_EmitFuncReturn(ModuleCompiler *m);
   int stackvalue_AnyRegUsedBySvalue(StackValue *sv);
   sljit_s32 pwart_GetFreeReg(ModuleCompiler *m, sljit_s32 regtype, int upstack);
   sljit_s32 pwart_GetFreeRegExcept(ModuleCompiler *m, sljit_s32 regtype,
                                    sljit_s32 except, int upstack);
   void pwart_EmitStackValueLoadReg(ModuleCompiler *m, StackValue *sv);
   int pwart_EmitFuncEnter(ModuleCompiler *m);
   ...
   char *pwart_PrepareFunc(ModuleCompiler *m);
   char *pwart_GenCode(ModuleCompiler *m);
   char *pwart_EmitFunction(ModuleCompiler *m, WasmFunction *func);
   void pwart_FreeFunction(WasmFunctionEntry code);
