...
namespace ewasvm
{
    ...
    char *load_module(ModuleCompiler *m, uint8_t *bytes, uint32_t byte_count)
    {
        ...
        m->context = (RuntimeContext *)wa_calloc(sizeof(RuntimeContext));
        ...
        while (pos < byte_count)
        {
            ...
            else if (id == 1)
            {
                ...
                for (uint32_t c = 0; c < count; c++)
                {
                    ...
                    if (count2 > 255)
                        return "type group too long.(should less than 256)";
            ...
            else if (id == 4)
            {
                ...
                for (int i = 0; i < table_count; i++)
                {
                    ...
                    table->entries = (void **)wa_calloc(table->size * sizeof(void *));
            ...
            else if (id == 5)
            {
                ...
                for (int i = 0; i < memory_count; i++)
                {
                    ...
                    if (memory->maximum == 0)
                    {
                        memory->bytes = (uint8_t *)wa_malloc(memory->initial * PAGE_SIZE);
                        memory->fixed = 0;
                    }
                    else
                    {
                        memory->bytes = (uint8_t *)wa_malloc(memory->maximum * PAGE_SIZE);
                        memory->fixed = 1;
                    }
            ...
            else if (id == 7)
            {
                ...
                for (uint32_t e = 0; e < export_count; e++)
                {
                    ...
                    if (strlen > 256)
                    {
                        return "export name too long, should less than 256 byte.";
                    }
            ...
            else if (id == 10)
            {
                ...
                m->context->funcentries = (WasmFunctionEntry *)wa_malloc(m->context->funcentries_count * sizeof(WasmFunctionEntry));
            ...
            if (tab->elem_type == WVT_FUNC)
            {
                for (int i = 0; i < tab->size; i++)
                {
                    if (tab->entries[i] != NULL)
                    {
                        ...
                        tab->entries[i] = (void *)((WasmFunction *)tab->entries[i])->func_ptr;
                    }
                }
            }
        }
